...
DEFAULT_ZIP_PATH = APP_ROOT / "wiktionary general pipeline" / "rawjsonforconversion" / "lexica-master.zip"
...
def _extract_pos(entry: ET.Element, primary_orth: ET.Element, english_fallback: str) -> str:
    ...
    if has_itype:
        ...
        if orth_text.endswith("ω") or orth_text.endswith("ομαι") or orth_text.endswith("μαι") or orth_text.endswith("μι"):
            return "verb"
        return "adj"
...
def _extract_forms(entry: ET.Element, orths: list[tuple[str, str, ET.Element]], primary_index: int) -> list[tuple[str, str, str]]:
...
def _build_parsed_entry(entry: ET.Element, *, entry_id: str) -> ParsedEntry | None:
    ...
    glosses_json = json.dumps([{"glosses": gloss_list}] if gloss_list else [], ensure_ascii=False, separators=(",", ":"))
    forms_json = json.dumps([[form_text, tag, roman] for form_text, tag, roman in forms], ensure_ascii=False, separators=(",", ":"))
    return ParsedEntry(
        headword=headword,
...
def _iter_xml_names(zf: zipfile.ZipFile) -> list[str]:
    return sorted(
        ...
        if name.endswith(".xml") and "/grc/lsj/" in name
    )
        ...
        for xml_name in xml_names:
            ...
            with zf.open(xml_name) as fh:
                for _event, elem in ET.iterparse(fh, events=("end",)):
                    ...
                    entry_id_base = str(elem.attrib.get("id", "") or elem.attrib.get("key", "") or "").strip()
...
def _batch_normalize(pairs: list[tuple[str, str]]) -> dict[tuple[str, str], str]:
    ...
    for start in range(0, len(unique_pairs), NORMALIZE_BATCH_SIZE):
        ...
        print(f"  Normalized {min(start + len(chunk), len(unique_pairs)):,} / {len(unique_pairs):,} texts...")
    return out
...
def _write_database(db_path: Path, zip_path: Path, entries: list[ParsedEntry]) -> tuple[int, int]:
    ...
    entry_batch: list[tuple[str, str, str, str, str, str, str, str, str, int, str, str, str, str]] = []
...
def main() -> int:
    ...
    print(f"Done: {entry_count:,} entries, {form_count:,} forms -> {db_path} ({size_mb:.1f} MB) in {elapsed:.1f}s")
    return 0
