...
def _make_session(ort: Any, session_path: str, profile: Dict[str, Any]) -> Any:
    ...
    return ort.InferenceSession(
        session_path, sess_options=options, providers=["CPUExecutionProvider"]
    )
...
def _benchmark_profile(
    ...
    return {
        ...
        "ort_session_mean_seconds": statistics.fmean(ort_session_seconds)
        if ort_session_seconds
        else None,
...
def main() -> int:
    ...
    session_path = str(
        (shim.metrics() or {}).get("onnx_session") or install_report.get("onnx_session") or ""
    )
    if not session_path:
    ...
    valid = [
        row for row in results if row.get("ok") and isinstance(row.get("p95_seconds"), (int, float))
    ]
    if not valid:
        raise RuntimeError(f"No ORT profile benchmark succeeded: {results}")

    baseline = next(
        (row for row in valid if (row.get("profile") or {}).get("name") == "ort_default"), valid[0]
    )
    best = min(
        valid, key=lambda row: float(row.get("p95_seconds") or row.get("mean_seconds") or 999999.0)
    )
    ...
    savings_pct = (
        ((baseline_seconds - best_seconds) / baseline_seconds * 100.0) if baseline_seconds else 0.0
    )
