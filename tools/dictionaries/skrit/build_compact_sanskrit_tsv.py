...
def sanitize_text(text: str) -> str:
    ...
    return " ".join(
        str(text or "").replace("\t", " ").replace("\r", " ").replace("\n", " ").split()
    ).strip()
...
def normalize_pos(
    ...
    if "conjunction" in first_gloss_lc:
        return "conj"
    if (
        "adverb" in first_gloss_lc
        or "indeclinable" in first_gloss_lc
        or "indecl." in first_gloss_lc
    ):
        return "adv"
    ...
    if "nominal" in form_types:
        return "noun"
    if (
        first_gloss_lc.startswith("see ")
        or first_gloss_lc.startswith("[cf.")
        or first_gloss_lc.startswith("cf. ")
    ):
        return ""
