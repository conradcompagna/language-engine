...
def load_source_entries(conn: sqlite3.Connection) -> list[EntryRow]:
    ...
    for row in conn.execute("SELECT * FROM entries ORDER BY headword, id"):
        ...
        entry.usable_gloss_texts = [
            text for text in entry.gloss_texts if is_usable_gloss_text(text)
        ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--source-db", type=Path, default=SOURCE_DB_PATH, help="Input LSJ SQLite database"
    )
    ...
    parser.add_argument(
        ...
        help="Output regenerated SQLite database",
    )
    parser.add_argument(
        "--cleanup-report",
        type=Path,
        default=DEFAULT_CLEANUP_REPORT_PATH,
        help="TSV report of deletions and redirects",
    )
    parser.add_argument(
        "--summary", type=Path, default=DEFAULT_SUMMARY_PATH, help="JSON summary of the rebuild"
    )
    return parser
...
def main() -> int:
    ...
    with args.tsv.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            target_headword = (
                headword
                if headword in canonical_survivor_by_headword
                else redirect_headword_map.get(headword)
            )
