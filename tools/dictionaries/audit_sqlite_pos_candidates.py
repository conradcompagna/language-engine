...
def _audit_one(path: Path, out_root: Path) -> dict[str, object]:
    ...
    conn = _open_db(path)
    try:
        summary["entry_count"] = int(
            conn.execute("SELECT COUNT(*) FROM entries").fetchone()[0] or 0
        )
        ...
        summary["top_pos"] = [
            {"pos": str(pos or ""), "count": int(count)} for pos, count in pos_rows[:20]
        ]
        ...
        summary["candidate_pos_counts"] = dict(
            sorted(pos_counter.items(), key=lambda item: (-item[1], item[0]))
        )
...
def _write_root_outputs(
    out_root: Path, summaries: list[dict[str, object]], generated_at: str
) -> None:
    (out_root / "overall_summary.json").write_text(
        json.dumps(
            {"generated_at": generated_at, "databases": summaries}, ensure_ascii=False, indent=2
        ),
        encoding="utf-8",
    )
        ...
        tsv_rows.append(
            {
                ...
                "candidate_pos_counts": json.dumps(
                    summary["candidate_pos_counts"], ensure_ascii=False, sort_keys=True
                ),
                "recommendation_counts": json.dumps(
                    summary["recommendation_counts"], ensure_ascii=False, sort_keys=True
                ),
    ...
    ranked = sorted(
        summaries, key=lambda row: (-int(row.get("candidate_entry_count") or 0), str(row["db"]))
    )
    lines = [
    ...
    for summary in ranked:
        ...
        lines.append(
            f"| `{summary['db']}` | `{int(summary.get('candidate_entry_count') or 0):,}` | `{bucket_text}` |"
        )
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit suspicious POS categories in SQLite dictionaries"
    )
    ...
    out_root = (
        Path(args.output_dir) if args.output_dir else REPORTS_DIR / f"sqlite_pos_probe_{timestamp}"
    )
