...
def _should_skip_korean_krdict_row(
    lang_code: str, source_label: str, pos: str, glosses_raw: str
) -> bool:
...
def convert_one(lang_code: str, tsv_path: Path, source_label: str = "default") -> Path:
    ...
    print(
        f"  [{lang_code}{suffix}] Reading {tsv_path.name} ({tsv_path.stat().st_size / 1e6:.1f} MB)..."
    )
            ...
            try:
                ...
                for form_item in forms_list:
                    if isinstance(form_item, list) and len(form_item) >= 1:
                        ...
                        form_text = str(
                            form_item.get("form") or form_item.get("text") or ""
                        ).strip()
                    ...
                    if form_text:
                        morph_parts = {
                            part.strip().lower() for part in morph.split(";") if part.strip()
                        }
    ...
    print(
        f"  [{lang_code}{suffix}] Done: {entry_count:,} entries, {form_count:,} forms -> {db_path.name} ({size_mb:.1f} MB)"
    )
    return db_path
