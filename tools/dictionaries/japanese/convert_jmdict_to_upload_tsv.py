...
def _map_pos(raw_pos_list: Sequence[str]) -> str:
    ...
    if (
        ...
        or any(
            tag.startswith("v1")
            or tag.startswith("v5")
            or tag.startswith("vk")
            or tag.startswith("vs")
            for tag in tags
        )
    ):
        return "verb"
...
def _reading_allowed_for_kanji(
    reading_restr: Dict[str, List[str]], reading: str, kanji: str
) -> bool:
        ...
        for kanji in kanji_forms:
            ...
            for reading in readings:
                if not _reading_allowed_for_kanji(reading_restr, reading, kanji):
                    continue
                if any(
                    _sense_applies_to_pair(sense, reading_restr, kanji, reading)
...
def _build_row_forms(
    ...
    if row_reading and row_reading != headword:
        reading_tag = (
            "search-only;reading"
            if has_search_only(reading_info.get(row_reading) or [])
            else "reading"
        )
    ...
    for kanji in applicable_kanji:
        if kanji != headword:
            kanji_tag = (
                "search-only;kanji"
                if has_search_only(kanji_info.get(kanji) or [])
                else "alternative;kanji"
            )
    ...
    for reading in applicable_readings:
        ...
        reading_tag = (
            "search-only;reading"
            if has_search_only(reading_info.get(reading) or [])
            else "alternative;reading"
        )
...
def convert(input_path: Path = INPUT_PATH, output_path: Path = OUTPUT_PATH) -> Dict[str, int]:
    ...
    with output_path.open("w", encoding="utf-8", newline="") as out:
        ...
        for entry in _parse_entries(xml_text):
            entry_count += 1
            kanji_forms = [
                str(v or "").strip() for v in entry["kanji_forms"] if str(v or "").strip()
            ]
            ...
            for group in pos_groups:
                ...
                headword, row_reading = _select_group_headword(
                    applicable_kanji, applicable_readings
                )
                if not headword:
                    continue
