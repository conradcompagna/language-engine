...
def main():
    ...
    entries = db.execute("SELECT id, headword, romanization, pos FROM entries").fetchall()
        ...
        if variant_is_kana and target_has_kanji:
            ...
            for entry_id, headword, pos, roman in target_entries:
                ...
                if kana_reading and kana_reading == variant_word:
                    ...
                    new_forms.append((entry_id, variant_word, form_key, "redirect;alternative", ""))
    ...
    for r in rows:
        print(
            f"  entry {r[0]} ({r['headword']}/{r['romanization']}/{r['pos']}) <- {r['form_text']}"
        )
