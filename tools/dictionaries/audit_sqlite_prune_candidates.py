...
def _looks_meta_form(form_text: str, morph_tags: str) -> str:
    ...
    if len(text.split()) >= 4 and any(
        token in folded for token in ("for other", "quoted by", "appendix")
    ):
        return "note-text"
...
def _scan_entries(conn: sqlite3.Connection, out_dir: Path, summary: DbSummary) -> None:
    ...
    if non_latin_rows:
        _write_tsv(
            out_dir / "headwords_non_latin_only_glosses.tsv", headword_headers, non_latin_rows
        )
    if relation_rows:
...
def _scan_duplicate_form_groups(
    conn: sqlite3.Connection, out_dir: Path, summary: DbSummary
) -> None:
    sql = """
        SELECT
            f.entry_id,
    ...
    """
    rows: list[dict[str, object]] = []
    for entry_id, headword, pos, form_text, morph_tags, romanization, dup_count in conn.execute(
        sql
    ):
...
def _scan_forms(
    ...
    for row in _iter_query(conn, sql):
        ...
        if head_chars and form_chars and head_chars.isdisjoint(form_chars):
            ...
            if (
                head_script == form_script
                and head_script not in {"Mixed", "Unknown"}
                and not _is_cjk_script(head_script)
            ):
    ...
    candidate_texts = {
        str(row["form_text"]) for row in meta_rows + no_shared_same_rows + no_shared_cross_rows
    }
...
def _write_high_fanout_groups(
    out_dir: Path, summary: DbSummary, high_fanout: dict[str, tuple[int, int]]
) -> None:
    rows = [
        {
            "form_text": form_text,
...
def _write_db_summary(out_dir: Path, summary: DbSummary) -> None:
    ...
    if summary.top_high_fanout:
        lines.extend(
            [
                "",
                "## Top High-Fanout Forms",
                "",
                "| Form | Entry Count | Row Count |",
                "| --- | ---: | ---: |",
            ]
        )
    ...
    if summary.top_meta_forms:
        lines.extend(
            ["", "## Top Meta/Note Forms", "", "| Form | Reason | Rows |", "| --- | --- | ---: |"]
        )
...
def main() -> int:
    ...
    output_dir = (
        ...
        if args.output_dir
        else REPORTS_DIR / f"sqlite_prune_probe_{timestamp}"
    )
