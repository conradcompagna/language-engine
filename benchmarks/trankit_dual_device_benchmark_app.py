...
CPU_OPT_DEPS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".trankit_cpu_opt_deps"
)
CPU_OPT_CACHE_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".trankit_cpu_opt_cache"
)
...
ONNX_DEPS_DIRS = [
    ...
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".trankit_onnx_deps"),
    CPU_OPT_DEPS_DIR,
]
ONNX_CACHE_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), ".trankit_compressed_runtime"
)
...
def _annotation_fingerprint(annotation: Any) -> str:
    payload = json.dumps(
        annotation, sort_keys=True, ensure_ascii=False, separators=(",", ":"), default=str
    )
...
def _annotation_discrepancy_report(cpu_annotation: Any, gpu_annotation: Any) -> Dict[str, Any]:
    ...
    sentence_token_same = (
        _sequence_same_count(cpu_sentence_sigs, gpu_sentence_sigs) if sentence_token_total else 0
    )
    metrics.append(
        _metric_payload("Sentence token sequences", sentence_token_same, sentence_token_total)
    )
...
def _write_cpu_opt_selection_cache(selection: Dict[str, Any]) -> None:
    ...
    payload = {
        ...
        "selected_tok_batch_size": int(
            selection.get("selected_tok_batch_size") or CPU_OPT_DEFAULT_TOK_BATCH_SIZE
        ),
        "selected_tag_batch_size": int(
            selection.get("selected_tag_batch_size") or CPU_OPT_DEFAULT_TAG_BATCH_SIZE
        ),
...
def _temporary_trankit_task_breakdown(
    ...
    def add_batch_stats(
        ...
        if isinstance(item_count, int) and item_count >= 0:
            ...
            row["batch_min_items"] = (
                item_count if previous_min is None else min(int(previous_min), item_count)
            )
            previous_max = row.get("batch_max_items")
            row["batch_max_items"] = (
                item_count if previous_max is None else max(int(previous_max), item_count)
            )
        if isinstance(padded_length, int) and padded_length >= 0:
            ...
            row["batch_max_padded_length"] = (
                padded_length if previous_len is None else max(int(previous_len), padded_length)
            )
            if isinstance(item_count, int) and item_count >= 0:
    ...
    def patch_bound_method(obj: Any, method_name: str, kind: str, count_as_batch: bool) -> None:
        ...
        def wrapped(*args: Any, **kwargs: Any):
            ...
            try:
                ...
                if stage_name:
                    add_batch_stats(
                        stage_name, kind, args, time.perf_counter() - started, count_as_batch
                    )
...
def _register_single_language_for_probe(lr: Any, lang_code: str) -> Dict[str, Any]:
    ...
    if treebank and name and name not in supported_langs:
        supported_langs.add(name) if isinstance(supported_langs, set) else supported_langs.append(
            name
        )
    ...
    try:
        ...
        if mode == "xlm_only":
            ...
            with timed_stage(
                ...
                pipeline_obj.eval()
            loaded_description = (
                "Shared XLM-R tokenizer/model only. No Trankit language modules are loaded."
            )
        elif mode == "full":
            loaded_description = (
                "Full all-language app Trankit pipeline via language_registry.init_trankit()."
            )
            ...
            loaded_description = (
                "Selected language-specific Trankit modules only. Shared XLM-R is not loaded."
            )
            ...
            with timed_stage("load_selected_language_modules", lang=resolved_lang):
                ...
                from trankit.config import config as master_config  # type: ignore
                from trankit.models.classifiers import (
                    NERClassifier,
                    PosDepClassifier,
                    TokenizerClassifier,
                )  # type: ignore
        ...
        if warmup_text.strip() and mode != "xlm_only" and mode != "lang_modules":
            if mode == "full":
                with timed_stage(
                    "first_inference_after_cold_load", lang=resolved_lang, chars=len(warmup_text)
                ):
            ...
            with _temporary_no_gc_collect() as gc_counter:
                with _temporary_no_cuda_empty_cache(torch_module) as cuda_counter:
                    with _temporary_tensor_sync_point_counter(torch_module) as sync_counter:
                        with _torch_cpu_inference_context(
...
def _install_onnx_xlmr_runtime(pipeline_obj: Any, torch_module: Any) -> Dict[str, Any]:
    ...
    if pipeline_obj is None:
        ...
        return report
    try:
        from trankit_compressed_runtime import install_compressed_runtime  # type: ignore
    except Exception as exc:
...
def _load_xlmr_int8_cache(pipeline_obj: Any, torch_module: Any) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if (
            ...
            and tuple(payload.get("excluded_path_tokens") or ()) != CPU_OPT_QUANTIZE_EXCLUDE_TOKENS
        ):
...
def _apply_cpu_optimization_profile(
    ...
    if profile in {CPU_OPT_PROFILE_NAME, "xlmr_int8", "xlmr_seq2seq_int8"}:
        try:
            ...
            report["xlmr_adapter_dynamic_int8_count"] = _count_dynamic_int8_modules_matching_path(
                xlmr_after, "adapter"
            )
        except Exception as exc:
...
def _install_onnx_dynamic_batching_patch() -> Dict[str, Any]:
    ...
    def choose_batch_size(
        lengths: list[int], candidates: list[int], requested: Optional[int], overhead_units: int
    ) -> Dict[str, Any]:
        clean_lengths = [
            int(length) for length in lengths if isinstance(length, int) and length > 0
        ]
    ...
    try:
        ...
        def dynamic_loader(dataset: Any, *args: Any, **kwargs: Any) -> Any:
            ...
            batch_indices = (
                selected_plan.get("batch_indices") if isinstance(selected_plan, dict) else None
            )
...
def _startup_tune_cpu_opt_runtime(
    ...
    _apply_cpu_opt_batch_sizes(
        pipeline_obj, CPU_OPT_DEFAULT_TOK_BATCH_SIZE, CPU_OPT_DEFAULT_TAG_BATCH_SIZE
    )
    try:
    ...
    try:
        ...
        if optimized_cpu:
            ...
            if optimization_report.get("errors"):
                raise RuntimeError(
                    "; ".join(str(error) for error in optimization_report.get("errors") or [])
                )
            ...
            if (
                ...
                startup_selection_payload = {
                    ...
                    "selected_tok_batch_size": int(
                        ...
                        or CPU_OPT_DEFAULT_TOK_BATCH_SIZE
                    ),
                    "selected_tag_batch_size": int(
                        startup_tuning_report.get("selected_tag_batch_size")
                        or CPU_OPT_DEFAULT_TAG_BATCH_SIZE
                    ),
            ...
            try:
                baseline_fingerprint = str(
                    (startup_validation_report.get("baseline") or {}).get("fingerprint") or ""
                )
                optimized_fingerprint = str(
                    (startup_validation_report.get("optimized") or {}).get("fingerprint") or ""
                )
            except Exception:
                pass
            ...
            cpu_opt_thread_count = int(
                startup_tuning_report.get("selected_thread_count") or cpu_opt_thread_count or 1
            )
        ...
        if optimized_gpu:
            ...
            if gpu_optimization_report.get("errors"):
                raise RuntimeError(
                    "; ".join(str(error) for error in gpu_optimization_report.get("errors") or [])
                )
            ...
            try:
                ...
                if onnx_cpu:
                    onnx_runtime_manager = getattr(
                        getattr(lr, "_trankit_pipeline", None), "_compressed_xlmr_runtime", None
                    )
                ...
                sequence_bucket = _sequence_bucket_for_token_count(
                ...
                inference_metrics = {
                    ...
                    "rss_peak_sampled_delta_bytes": _bytes_delta(
                ...
                if optimized_cpu:
                    ...
                    inference_metrics["used_bf16_autocast"] = _cpu_profile_uses_bf16(
                        cpu_opt_profile
                    )
                if optimized_gpu:
                            ...
                            if isinstance(onnx_runtime_metrics_before, dict):
                                ...
                                if isinstance(after_by_key, dict):
                                    ...
                                    for key, after_row in after_by_key.items():
                                        ...
                                        before_row = (
                                            before_row if isinstance(before_row, dict) else {}
                                        )
...
def _cpu_opt_candidate_worker_main(
    ...
    try:
        deps_path_active = (
            profile in {"ipex_xlmr", "onnx_xlmr", "onnx_int8_xlmr"} and _prepend_cpu_opt_deps()
        )
                ...
                result_queue.put(
                    {
                        ...
                        "error": str(
                            bf16_probe.get("reason") or "CPU BF16 support was not detected"
                        ),
...
def _select_cpu_optimization_profile(command: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if valid_threads:
        baseline = min(
            valid_threads, key=lambda row: float(row.get("elapsed_seconds") or float("inf"))
        )
    else:
        baseline = {
            "ok": False,
    ...
    if valid_candidates:
        selected = min(
            valid_candidates, key=lambda row: float(row.get("elapsed_seconds") or float("inf"))
        )
    else:
        selected = baseline
    ...
    return {
        ...
        "output_match": bool(
            selected.get("output_fingerprint") == baseline_fingerprint and baseline_fingerprint
        ),
...
def api_analyze_gpu_opt_vs_gpu() -> Any:
    if not {"gpu_opt", "gpu"}.issubset(workers):
        return jsonify(
            {"ok": False, "error": "optimized GPU and regular GPU workers are not started"}
        ), 503
    for worker in workers.values():
        worker.drain()
    ...
    gpu_metrics = gpu_result.get("metrics") if isinstance(gpu_result, dict) else None
    gpu_opt_seconds = (
        gpu_opt_metrics.get("elapsed_seconds") if isinstance(gpu_opt_metrics, dict) else None
    )
    gpu_seconds = gpu_metrics.get("elapsed_seconds") if isinstance(gpu_metrics, dict) else None
    speedup = None
    if (
    ...
    gpu_opt_fingerprint = (
        gpu_opt_metrics.get("output_fingerprint") if isinstance(gpu_opt_metrics, dict) else None
    )
    gpu_fingerprint = (
        gpu_metrics.get("output_fingerprint") if isinstance(gpu_metrics, dict) else None
    )
...
def api_analyze_onnx_cpu_vs_gpu() -> Any:
    ...
    onnx_fingerprint = (
        onnx_metrics.get("output_fingerprint") if isinstance(onnx_metrics, dict) else None
    )
    gpu_fingerprint = (
        gpu_metrics.get("output_fingerprint") if isinstance(gpu_metrics, dict) else None
    )
...
def wait_for_required_workers(required_names: list[str]) -> None:
    ...
    last_line = ""
    print(
        "Waiting for benchmark workers to fully load before serving: " + ", ".join(required_names),
        flush=True,
    )
    while time.time() < deadline:
        for worker in workers.values():
        ...
        line = " | ".join(
            _startup_status_line(name, statuses.get(name, {})) for name in required_names
        )
        if line != last_line:
    ...
    timed_out = {name: workers[name].status for name in required_names if name in workers}
    raise RuntimeError(
