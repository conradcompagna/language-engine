...
COMMON_FORMAT = (
    "LAS={:.4f},UAS={:.4f},LAS_POS={:.4f},UAS_POS={:.4f},POS={:.4f},TAG={:.4f},boundary={:.4f}"
)
    ...
    def print(self, file=sys.stdout):
        ...
        for title, matrix in (
            ...
            for gold, results in sorted(matrix.items(), key=lambda t: t[0]):
                results = matrix[gold]
                print(
                    ("  {:<" + max_label_len + "}({:>6}): {}").format(
                        gold,
                        sum(results.values()),
                        ", ".join(
                            [
                                "{}={}".format(pos, num)
                                for pos, num in sorted(results.items(), key=lambda t: -t[1])
                            ]
                        ),
                    ),
                    file=file,
            ...
            for gold, results in sorted(matrix.items(), key=lambda t: t[0]):
                ...
                output = sum(
                    sum(v for k, v in r.items() if k.lower() == gold.lower())
                ...
                print(
                    ("  {:<" + max_label_len + "}: {:.3f}, {:.3f}, {:.3f}").format(gold, p, r, f),
                    file=file,
                )

        print(
            "sentence={}, gold_token={}, result_token={}, custom_cond={:.4f}({}/{})".format(
                self.sentences,
                self.gold_tokens,
                self.result_tokens,
        ...
        print(
            ("        token_f1:" + COMMON_FORMAT).format(
                ...
                f1(
                    self.correct_tokens / self.gold_tokens, self.correct_tokens / self.result_tokens
                ),
            ),
            file=file,
        )
        print(
            ("    token_recall:" + COMMON_FORMAT).format(
                self.correct_las_tokens / self.gold_tokens,
                self.correct_uas_tokens / self.gold_tokens,
                self.correct_pos_las_tokens / self.gold_tokens,
                self.correct_pos_uas_tokens / self.gold_tokens,
        ...
        print(
            (" token_precision:" + COMMON_FORMAT).format(
                self.correct_las_tokens / self.result_tokens,
                self.correct_uas_tokens / self.result_tokens,
                self.correct_pos_las_tokens / self.result_tokens,
                self.correct_pos_uas_tokens / self.result_tokens,
        ...
        print(
            ("  whole_sentence:" + COMMON_FORMAT + ",root={:.4f}").format(
        ...
        if self.gold_ents and self.result_ents:
            print(
                "        ent_f1:SPAN_LABEL={:.4f},SPAN_ONLY={:.4f}".format(
                    f1(
            ...
            print(
                "    ent_recall:SPAN_LABEL={:.4f},SPAN_ONLY={:.4f}".format(
                    self.correct_ent_labels / self.gold_ents,
                    self.correct_ent_spans / self.gold_ents,
                ),
                file=file,
            ...
            print(
                " ent_precision:SPAN_LABEL={:.4f},SPAN_ONLY={:.4f}".format(
                    self.correct_ent_labels / self.result_ents,
                    self.correct_ent_spans / self.result_ents,
                ),
                file=file,
            ...
            if debug:
                if g_end == r_end:
                    print(
                        "{}\t{}\t{}".format(
                            "="
                            if index_g == last_match_g and index_r == last_match_r
                            else ">"
                            if index_g == last_match_g
                            else "<"
                            if index_r == last_match_r
                            else "!",
                            ",".join(
                                [
                                    "-".join(
                                ...
                                [
                                    "-".join(
                                        (
                                            ...
                                            m.dep_,
                                            str(m.head.idx),
                                            str(m.head.idx + len(m.head.orth_)),
                                        )
                                    )
...
def is_correct_dep(g, r):
    return (
        g["head"]["offset"] <= r.head.idx
        and g["head"]["end"] >= r.head.idx + len(r.head.orth_)
        or g["head"]["offset"] >= r.head.idx
        and g["head"]["end"] <= r.head.idx + len(r.head.orth_)
    )
