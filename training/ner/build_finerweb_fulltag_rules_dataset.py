...
class KnnMatcher:
    def __init__(self, top500_rows, vectors):
        ...
        self.matrix = (
            np.vstack([a["vec"] for a in anchors])
            if anchors
...
def hard_head_override(label):
    ...
    if coarse in {"person", "persons", "pessoa", "บุคคล", "شخص", "אדם", "individual"}:
        if contains_any(
            text, ["deity", "god", "goddess", "orix", "mythological", "avatar", "saint"]
        ):
...
def override_bucket(label, mapping_text, current_bucket):
    ...
    if contains_any(
        text,
        ["hotel code", "airport code", "airline code", "code/identifier", "document identifier"],
    ):
    ...
    if contains_any(
        text, ["hotel rating", "rating hotel", "earthquake magnitude", "magnitude earthquake"]
    ):
    ...
    if contains_any(text, ["object"]) and not contains_any(
        text, ["natural object", "astronomical object", "celestial object", "celestial body"]
    ):
    ...
    if contains_any(
        text, ["industrial complex", "bus operator", "rail operator", "airport operator"]
    ):
    ...
    if contains_any(
        text, ["professional organization", "professional association", "professional body"]
    ):
    ...
    if contains_any(
        ...
        if not contains_any(
            text,
            ["organization", "club", "league ranking", "ranking", "score", "format", "category"],
        ):
...
def build_rule_map(label_map, matcher):
    ...
    for label, row in label_map.items():
        ...
        if source == "fulltag_base" and has_fine and coarse in WEAK_HEADS:
            bucket, source, semantic_text, knn_meta = matcher.map_label(
                label, row["new_main_bucket"]
            )
...
def surface_bucket_override(lang, surface, original_label, mapped_bucket):
    ...
    if CELESTIAL_RE.search(text) or contains_any(
        label, ["celestial body", "astronomical object", "planet", "asteroid", "galaxy", "nebula"]
    ):
...
def convert_split(src, dst_bio, dst_side, rule_map, lang):
    ...
    with (
        ...
        dst_bio.open("w", encoding="utf-8", newline="") as fbio,
        dst_side.open("w", encoding="utf-8", newline="") as fside,
    ):
        ...
        def flush_sentence():
            ...
            if lang == "tha":
                ...
                while i < len(sentence_rows):
                    ...
                    if pref != "B":
                        ...
                        continue
                    j = i + 1
                    while j < len(sentence_rows) and sentence_rows[j]["original_bio"].startswith(
                        "I-"
                    ):
                        j += 1
            ...
            while i < len(sentence_rows):
                ...
                if not any(k in span_ids_to_filter for k in range(i, j)):
                    ...
                    override_bucket, override_source = surface_bucket_override(
                        lang, surface, label, mapped["new_main_bucket"]
                    )
                    if override_bucket:
                        for k in range(i, j):
            ...
            for row_idx, row in enumerate(sentence_rows):
                ...
                if row_idx in span_ids_to_filter or orig == "O":
                    ...
                    mapping_text = "_"
                    source = (
                        "thai_single_char_span_filter" if row_idx in span_ids_to_filter else "_"
                    )
                    ...
                    if pref == "B":
                        ...
                        original_counts[
                            (mapped["original_label"], bucket, mapped["previous_bucket"], source)
                        ] += 1
...
def convert_dataset(rule_map):
                ...
                if f.tell() == 0:
                    writer.writerow(
                        [
                            ...
                            "entity_spans",
                            "entity_tokens",
                            "thai_single_char_span_tokens_filtered",
                        ]
                    )
                writer.writerow(
                    [
                        ...
                        stats["sentences"],
                        stats["tokens"],
                        sum(bucket_spans.values()),
                        sum(bucket_tokens.values()),
                        stats["thai_single_char_span_tokens_filtered"],
                    ]
                )
            if split == "all":
            ...
            json.dumps(
                {
                    ...
                    "thai_single_char_span_tokens_filtered": lang_stats[
                        "thai_single_char_span_tokens_filtered"
                    ],
                },
                ensure_ascii=False,
    ...
    report = {
        ...
        "thai_single_char_span_tokens_filtered": root_stats[
            "thai_single_char_span_tokens_filtered"
        ],
    }
...
def write_counts(root, bucket_tokens, bucket_spans, original_counts):
    ...
    with (root / "original_label_bucket_counts.tsv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
        writer.writerow(
            ["original_label", "new_main_bucket", "previous_bucket", "rule_source", "span_count"]
        )
...
def validate():
            ...
            with bio.open("r", encoding="utf-8", newline="") as f:
                for lineno, raw in enumerate(f, 1):
                    ...
                    if parts[1] != "O" and parts[1].split("-", 1)[1] not in EXPECTED_BUCKETS:
                        issues.append(
                            [lang, split, str(bio), lineno, "unexpected_bucket", parts[1]]
                        )
    ...
    report = {
        ...
        "issues_tsv": str(OUTPUT_ROOT / "archive_health_issues.tsv"),
    }
