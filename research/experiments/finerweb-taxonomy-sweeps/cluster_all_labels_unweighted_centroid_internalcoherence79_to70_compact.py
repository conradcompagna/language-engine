...
def write_coverage_89_to70(
    thresholds: list[int],
    clusters_by_threshold: dict[int, list[base.Cluster]],
    rows: list[dict[str, object]],
    total_count: int,
) -> None:
    path = OUT_DIR / "all_labels_unweighted_centroid_internalcoherence89_to70_compact_coverage.md"
    vectorized_count = sum(int(row["count"]) for row in rows)
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("# All Labels Unweighted Centroid Internal-Coherence 89-70 Coverage\n\n")
...
def main() -> None:
    ...
    all_thresholds = list(range(89, 69, -1))
    clusters_by_threshold: dict[int, list[base.Cluster]] = {}

    for threshold in range(89, 79, -1):
    ...
    for threshold in range(79, 69, -1):
    ...
    write_coverage_89_to70(all_thresholds, clusters_by_threshold, rows, total_count)
    print(f"out_dir={OUT_DIR}")
    print(f"coverage={OUT_DIR / 'all_labels_unweighted_centroid_internalcoherence89_to70_compact_coverage.md'}")
