...
FINAL_TAG_MAP = BASE / "derived_fasttext_categories" / "user_grouped_5050_no_cultural_final12_raw_tag_map.tsv"
FINAL_SUMMARY = BASE / "derived_fasttext_categories" / "user_grouped_5050_no_cultural_final12_raw_summary.tsv"
...
def map_original_bio(original_bio: str, label_map: dict[str, dict[str, str]]) -> tuple[str, str, str, str, str]:
    if original_bio == "O":
    ...
    if not bucket:
        return "O", mapped["coarse_label"], "_", "dropped_unretained_coarse", mapped.get("vector_region", "_")
    return f"{prefix}-{bucket}", mapped["coarse_label"], bucket, "retained", mapped.get("vector_region", "_")
...
def build_sentence_rows(
    ...
    for token, start, end in tokens:
        ...
        slim_bio, coarse_label, retained_label, status, vector_region = map_original_bio(original_bio, label_map)
...
def write_span_sentence(rows: list[dict[str, str]], span_handle) -> None:
    ...
    for idx, (bucket, span_rows) in enumerate(spans, start=1):
        ...
        span_handle.write(f"{idx}\t{span_text}\t_\t{bucket}\t{original_tags}\t_\t0\t_\t_\t{misc_origin}\n")
    span_handle.write("\n")
