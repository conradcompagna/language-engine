...
def write_anchor_summary(top500_rows, anchor_summary):
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow(["bucket", "manual_anchor_count", "manual_anchor_freq", "anchors_with_vectors", "anchor_freq_with_vectors"])
...
def convert_split(lang_dir, out_lang_dir, lang, split, label_map):
    ...
    with src.open("r", encoding="utf-8", newline="") as fin, bio_out.open(
        "w", encoding="utf-8", newline=""
    ) as fbio, side_out.open("w", encoding="utf-8", newline="") as fside:
        ...
        for lineno, raw in enumerate(fin, 2):
            ...
            if len(parts) != len(header):
                raise ValueError(f"Bad TSV row in {src}:{lineno}: expected {len(header)} columns, got {len(parts)}")
            ...
            if original_bio == "O":
                ...
                if prefix == "B":
                    ...
                    original_bucket_counts[(mapped["original_label"], bucket, mapping_text, assignment_source)] += 1
...
def convert_datasets(label_map, method_description):
    ...
    for lang in langs:
        ...
        write_lang_reports(out_lang_dir, lang, split_reports, lang_original_counts, lang_token_counts, lang_span_counts, lang_assignment_sources)
    ...
    write_root_reports(root_report, global_original_counts, global_label_token_counts, global_label_span_counts, global_assignment_sources)
    return root_report


def write_lang_reports(out_lang_dir, lang, split_reports, original_counts, token_counts, span_counts, source_counts):
    ...
    with (out_lang_dir / "original_label_bucket_counts.tsv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
        writer.writerow(["original_label", "new_main_bucket", "mapping_text", "assignment_source", "span_count"])
        for (label, bucket, mapping_text, source), count in sorted(original_counts.items(), key=lambda x: (-x[1], x[0])):
    ...
    (out_lang_dir / "dataset_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
...
def write_root_reports(root_report, original_counts, token_counts, span_counts, source_counts):
    ...
    with (OUTPUT_ROOT / "original_label_bucket_counts.tsv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
        writer.writerow(["original_label", "new_main_bucket", "mapping_text", "assignment_source", "span_count"])
        for (label, bucket, mapping_text, source), count in sorted(original_counts.items(), key=lambda x: (-x[1], x[0])):
            writer.writerow([label, bucket, mapping_text, source, count])

    with (OUTPUT_ROOT / "assignment_source_summary.tsv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f, delimiter="\t")
    ...
    (OUTPUT_ROOT / "dataset_report.json").write_text(json.dumps(root_report, ensure_ascii=False, indent=2), encoding="utf-8")
...
def validate_dataset():
    ...
    for lang in langs:
        for split in SPLITS:
                ...
                for lineno, line in enumerate(f, 1):
                    ...
                    if parts[1] != "O":
                        ...
                        if bucket not in EXPECTED_BUCKETS:
                            issues.append([lang, split, str(bio), lineno, "unexpected_bucket", bucket])
            ...
            with side.open("r", encoding="utf-8", newline="") as f:
                ...
                header = next(reader, [])
                if header != ["token", "original_bio", "mapping_text", "assignment_source", "bucket_bio"]:
                ...
                for lineno, parts in enumerate(reader, 2):
                    if not parts:
                        ...
                        continue
                    if len(parts) != 5:
                        issues.append([lang, split, str(side), lineno, "bad_side_columns", "\t".join(parts)[:80]])
                        continue
                    token, original_bio, _, _, bucket_bio = parts
                    if not BIO_RE.match(original_bio):
                        issues.append([lang, split, str(side), lineno, "bad_original_bio", original_bio])
                    if not LABEL_RE.match(bucket_bio):
                        issues.append([lang, split, str(side), lineno, "bad_side_bucket", bucket_bio])
    ...
    report = {"languages": langs, "files_checked": len(langs) * len(SPLITS) * 2, "issue_count": len(issues), "issues_tsv": str(issue_path)}
...
def build_variant(mode, output_root, top500_rows, manual_map, centroids, anchor_summary, vectors):
    ...
    summary = {
        ...
        "assignment_source_counts_by_label": dict(Counter(row["assignment_source"] for row in label_map.values())),
...
def main():
    ...
    summaries = [
        build_variant("fine", BASE / "datasets" / "finerweb_bucketed_fine_fasttext_by_language", top500_rows, manual_map, centroids, anchor_summary, vectors),
        build_variant("fulltag", BASE / "datasets" / "finerweb_bucketed_fulltag_fasttext_by_language", top500_rows, manual_map, centroids, anchor_summary, vectors),
    ]
