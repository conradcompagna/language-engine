...
builder.DATASET_NAME = "finerweb_plo75"
builder.DATASET_DISPLAY_NAME = "PLO75"
builder.DATASET_DESCRIPTION = (
    "Original fiNERweb labels are mapped through the 75% FastText retained map using only PERSON, LOCATION, "
    "and ORGANIZATION seed regions. Full original labels under each coarse tag provide vector evidence; no "
    "special cultural-reference child stripping is applied. Labels outside the retained map are dropped to `O`; "
    "sentences with no retained entity labels are omitted."
)

builder.OUTPUT_BY_LANG = builder.DATASETS / "finerweb_plo75_by_language"
builder.OUTPUT_COMBINED = builder.DATASETS / "finerweb_plo75"
...
builder.RETAINED_MAP = (
    ...
    / "finerweb_selected_manual_seed_regions_threshold75_plo_split_cultural_retained_tag_map.tsv"
)
