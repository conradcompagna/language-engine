...
def force_utf8_open(file, mode="r", buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None):
...
def watch_active_model(args: argparse.Namespace, initial_signature: tuple[int, int] | None, stop: threading.Event) -> None:
...
def patch_trankit_ner_eval_logging() -> None:
    ...
    def _eval_ner(self, data_set, batch_num, name, epoch):
        ...
        if rows:
            ...
            for row in rows:
                self._printlog(
                    "{label}\t{gold}\t{pred}\t{correct}\t{precision:.2f}\t{recall:.2f}\t{f1:.2f}".format(**row)
                )
        return score
