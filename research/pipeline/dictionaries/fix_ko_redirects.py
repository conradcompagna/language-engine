...
def _load_entries(db: sqlite3.Connection) -> list[sqlite3.Row]:
    return db.execute(
        "SELECT id, headword, romanization, pos FROM entries ORDER BY id"
    ).fetchall()
...
def _parse_forms_json(raw: str) -> list[list[str]]:
    ...
    for item in parsed:
        if isinstance(item, list) and item:
            ...
            tags = str(item.get("tags") or item.get("label") or item.get("commentary") or "").strip()
...
def _build_redirect_insert_rows(
    ...
    for parent_word, variant_word, variant_key in redirect_pairs:
        ...
        for entry in candidates:
            ...
            insert_rows.append(
                (entry_id, variant_word, variant_key, "redirect;alternative", "")
            )
            additions_by_entry[entry_id].append([variant_word, "redirect;alternative", ""])
            matched_pairs += 1

    return insert_rows, additions_by_entry, {
...
def _rewrite_entry_forms(
    ...
    for chunk in _chunked(sorted(affected_entry_ids)):
        ...
        for row in rows:
            ...
            filtered = [item for item in forms if len(item) >= 2 and str(item[1] or "").strip() != "redirect;alternative"]
...
def _sample_entry_report(db: sqlite3.Connection, headword: str) -> None:
    ...
    print(f"  Sample headword {json.dumps(headword, ensure_ascii=True)}: {len(rows):,} redirect rows")
    for row in rows[:20]:
        print(
            "    "
            + json.dumps(
