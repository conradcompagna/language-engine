...
SRC = os.path.join(os.path.dirname(__file__), "training", "oldeng", "bosworth toller old english.txt")
...
SKIP_TAGS_RE = re.compile(r"<(?:PAGE|HEADER|letterheader|INTRODUCTION|/INTRODUCTION)[^>]*>.*?(?:</(?:HEADER|letterheader|INTRODUCTION)>|$)", re.IGNORECASE | re.DOTALL)
...
def is_english_gloss(s):
    ...
    if not words:
        return False
    english_markers = {"a","an","the","to","of","in","on","at","for","with",
                       "and","or","but","not","is","was","be","been","being",
                       "has","have","had","one","who","that","which","any",
                       "some","by","from","as","it","its","this","these",
...
def parse_entry(para):
    ...
    for idx, ital in enumerate(italics):
        ...
        for sub in re.split(r":--", ital_clean):
            ...
            pattern = r"^(?:(?:" + r"ic|þú|ðú|he|heo|hit|pl\.|p\.|pp\.|pres\.|subj\.|imp\.|impert\.|gen\.|dat\.|acc\.|nom\.|voc\.|inst\.|comp\.|sup\.|superl\.|indecl\." + r")\s+[^\s;]+(?:,\s*[^\s;]+)*\s*;?\s*)+"
    ...
    POS_ABBRS = {"a", "trans", "intrans", "n", "refl", "impers", "a.", "trans.",
                 "intrans.", "n.", "refl.", "impers."}
...
def build():
    ...
    c.execute("INSERT INTO meta VALUES (?,?)", ("notes", "Supplementary. Acutes converted to macrons at build time."))
    ...
    for e in real_entries:
        ...
        c.execute("""
            ...
            VALUES (?,?,?,?,?,?,?)
        """, (e["headword"], e["pos"], glosses_json, forms_json, e["pos_tags"],
              "bosworth-toller", "compact"))
        entry_id = c.lastrowid
        for f, m in deduped:
            c.execute("INSERT INTO forms (entry_id, form_text, morph_tags) VALUES (?,?,?)",
                      (entry_id, f, m))
