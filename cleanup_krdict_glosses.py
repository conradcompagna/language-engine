...
def main() -> int:
    parser = argparse.ArgumentParser(description="Copy-edit KRDict gloss strings in ko-krdict.sqlite.")
    parser.add_argument(
        "db",
        nargs="?",
    ...
    try:
        ...
        print(
            f"Updated {row_changes:,} rows and {gloss_changes:,} gloss strings in {db_path.name}"
        )
        return 0
    finally:
        conn.close()
