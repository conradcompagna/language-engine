...
def extract_adapter_pack(
    torch: Any, pipeline: Any, original_load_adapter: Any, lang: str, task: str
) -> Dict[str, Any]:
    ...
    for path, adapter in adapters:
        if getattr(adapter, "add_layer_norm_before", False) or getattr(
            adapter, "add_layer_norm_after", False
        ):
    ...
    if len(set(residual_before_ln)) != 1:
        raise RuntimeError(
            f"Mixed adapter residual modes are not implemented: {residual_before_ln}"
        )
    ...
    return {
        ...
        "compressed_bytes": int(
            sum(item["values"].nbytes + item["scale"].nbytes for item in compressed.values())
        ),
    }
...
def patch_adapters_for_dynamic_int8_inputs(
    torch: Any, xlmr: Any, residual_before_ln: bool
) -> Dict[str, Any]:
...
def build_artifacts(pipeline: Any, torch: Any, sequence_length: int = 128) -> Dict[str, Any]:
    ...
    if len(signatures) != 1:
        raise RuntimeError(
            f"Adapter signatures are not uniform; cannot use one XLM-R graph: {len(signatures)} signatures"
        )
...
def load_ort_session_tuning(
    session_path: Path, tuning_path: Path = ORT_SESSION_TUNING_PATH
    ...
    try:
        ...
        if (
            ...
            and int(session_path.stat().st_size) != expected_bytes
        ):
...
def create_tuned_ort_session(ort: Any, session_path: Path) -> Tuple[Any, Dict[str, Any]]:
    ...
    if options is None:
        ...
        session = ort.InferenceSession(
            str(session_path), sess_options=options, providers=["CPUExecutionProvider"]
        )
    return session, tuning_report


def install_compressed_runtime(
    pipeline: Any, torch: Any, manifest_path: Path = MANIFEST_PATH
) -> Dict[str, Any]:
    prepend_deps()
    import numpy as np
    import onnxruntime as ort  # type: ignore
    ...
    if manifest.get("profile") != PROFILE_NAME:
        raise RuntimeError(
            f"Compressed runtime profile mismatch: {manifest.get('profile')} != {PROFILE_NAME}"
        )
    ...
    class ORTXLMRShim(torch.nn.Module):
        ...
        def _add_timing(
            ...
            row["preprocess_seconds"] = (
                float(row.get("preprocess_seconds") or 0.0) + preprocess_seconds
            )
            row["session_seconds"] = float(row.get("session_seconds") or 0.0) + session_seconds
            row["postprocess_seconds"] = (
                float(row.get("postprocess_seconds") or 0.0) + postprocess_seconds
            )

        def forward(
            self, input_ids: Any = None, attention_mask: Any = None, **_: Any
        ) -> Tuple[Any]:
