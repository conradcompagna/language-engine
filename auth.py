...
def signup():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"ok": False, "error": f"Could not send verification email: {type(exc).__name__}"}), 500
...
def resend_verification():
    ...
    if not user or not user.password_hash or user.email_verified_at:
        return jsonify({"ok": True, "message": "If this account needs verification, a new link has been sent."})
...
def login():
    ...
    if user.password_hash and not user.email_verified_at:
        return jsonify({"ok": False, "error": "Check your email to verify your account before signing in."}), 403
...
def forgot_password():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"Could not send reset email: {type(exc).__name__}"}), 500
    return jsonify({"ok": True})
...
def change_email_request():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"Could not send confirmation email: {type(exc).__name__}"}), 500
    return jsonify({"ok": True})
...
def delete_account():
    ...
    exec_sql("account_action_tokens", "DELETE FROM account_action_tokens WHERE user_id = :user_id", user_id=user_id)
    exec_sql("custom_entry_deletion_votes", "DELETE FROM custom_entry_deletion_votes WHERE user_id = :user_id", user_id=user_id)
    exec_sql("gemini_deletion_votes", "DELETE FROM gemini_deletion_votes WHERE user_id = :user_id", user_id=user_id)
    exec_sql("gemini_entry_owners", "DELETE FROM gemini_entry_owners WHERE user_id = :user_id", user_id=user_id)
    exec_sql("synthetic_annotations", "DELETE FROM synthetic_annotations WHERE user_id = :user_id", user_id=user_id)
    exec_sql("user_annotations", "DELETE FROM user_annotations WHERE user_id = :user_id", user_id=user_id)
    exec_sql("user_dict_entries", "DELETE FROM user_dict_entries WHERE user_id = :user_id", user_id=user_id)

    # Shared app/community content stays, but any stale account attribution is removed.
    exec_sql("custom_dict_entries", "UPDATE custom_dict_entries SET owner_user_id = NULL WHERE owner_user_id = :user_id", required_column="owner_user_id", user_id=user_id)
    exec_sql("entry_notes", "UPDATE entry_notes SET user_id = NULL WHERE user_id = :user_id", required_column="user_id", user_id=user_id)
    exec_sql("entry_decomps", "UPDATE entry_decomps SET user_id = NULL WHERE user_id = :user_id", required_column="user_id", user_id=user_id)
    ...
    if current_user.is_authenticated:
        return jsonify({
            ...
            "user": {
                ...
                "password_length": current_user.password_length if current_user.password_hash else None,
            },
        })
...
def google_callback():
    ...
    if not user:
        ...
        if user:
            ...
            # New user
            user = User(username=email, email=email, google_id=google_id, email_verified_at=_utcnow())
            db.session.add(user)
            db.session.flush()
            quota = LookupQuota(user_id=user.id)
            db.session.add(quota)
