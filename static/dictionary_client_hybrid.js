...
(function() {
  ...
  // Legacy two-pass lookup helper. Keep this disconnected from the active
  // compact-index -> hydrate -> render flow.
  function buildSurfaceLemmaAwareLookup(surfaceText, lemmaText, engine, upos, xpos, includeDebugTrace) {
    var surface = String(surfaceText || "").trim();
    var hintData = buildLemmaHintData(surface, lemmaText, engine, upos || "", xpos || "");
    var lemmaHints = hintData.lemma_hint_objects;
    var lemmaOracleUsed = hintData.lemma_differs_from_surface && lemmaHints.length > 0;
    var exactEntries = engine.lookup_all(surface) || [];
    var exactMatchInfo = lemmaOracleUsed
      ? collectEntriesMatchingLemmaHints(exactEntries, lemmaHints, engine)
      : { entries: [], matches: [] };
    var exactPreferredEntries = exactMatchInfo.entries.length ? exactMatchInfo.entries.slice() : exactEntries.slice();
    var oracleFill = null;

    function getLemmaOracleFill() {
      if (!lemmaOracleUsed) return null;
      if (oracleFill) return oracleFill;
      var oracleFillOpts = { allowExact: false, upos: upos || "" };
      if (includeDebugTrace) oracleFillOpts.debug = true;
      oracleFillOpts.lemma_hints = lemmaHints;
      oracleFill = engine.fill_token(surface, oracleFillOpts);
      if (oracleFill && String(oracleFill.mode || "").trim().toLowerCase() === "greedy") {
        oracleFill.mode = "lemma_greedy";
      }
      return oracleFill;
    }
    ...
    if (lemmaOracleUsed) {
      var useDirectMwtLemmaMapping = mwtParts.length > 1;
      ...
      if (alignedLemmaPartsResult) {
        ...
        if (alignedResolvedVia === "lemma_partial_override") {
          alignedLemmaPartsResult.resolution_meta = buildResolutionMeta(
            ...
            partialGapCount > 0
              ? [
                  buildResolutionRouteStep("surface_exact_lookup", "surface exact lookup missed"),
                  buildResolutionRouteStep("lemma_exact_parts", "exact lemma parts were aligned onto the surface"),
                  buildResolutionRouteStep("lemma_gap_greedy", "greedy DP filled the remaining uncovered spans")
                ]
              : [
                  buildResolutionRouteStep("surface_exact_lookup", "surface exact lookup missed"),
                  buildResolutionRouteStep("lemma_exact_parts", "exact lemma parts were aligned onto the surface"),
                  buildResolutionRouteStep("lemma_gap_greedy", "no uncovered spans remained after exact lemma-part alignment")
                ],
            "lemma_partial_override",
          ...
          alignedLemmaPartsResult.resolution_meta = buildResolutionMeta(
            ...
            [
              ...
              buildResolutionRouteStep("lemma_override", "lemma exact-part alignment selected")
            ],
            "lemma_override",
    ...
    var fill = null;
    if (lemmaOracleUsed) {
      fill = getLemmaOracleFill();
    } else {
      var fillOpts = { allowExact: false, upos: upos || "" };
      if (includeDebugTrace) fillOpts.debug = true;
      fill = engine.fill_token(surface, fillOpts);
    }

    if (fill && fill.has_known) {
      var promoted = fill.has_lemma_promotion ? collectPromotedFillEntries(fill) : [];
      var allEntries = promoted.length ? promoted : collectFillEntries(fill);
      if (allEntries.length) {
        var fillMode = String((fill && fill.mode) || "greedy").trim().toLowerCase();
        var resolutionCategory = promoted.length ? "greedy_lemma_match" : "greedy_segmentation";
        var resolutionRouteCode = promoted.length ? "surface_greedy_lemma_match" : "surface_greedy";
        var resolutionRouteSteps = [
          buildResolutionRouteStep("surface_exact_lookup", "surface exact lookup missed")
        ];
        if (promoted.length) {
          resolutionRouteSteps.push(
            buildResolutionRouteStep("lemma_greedy", "lemma-aware greedy DP selected lemma-linked pieces")
          );
        } else if (lemmaOracleUsed) {
          resolutionRouteSteps.push(
            buildResolutionRouteStep("lemma_greedy", "lemma-aware greedy DP selected a surface path")
          );
        } else {
          resolutionRouteSteps.push(
            buildResolutionRouteStep("surface_greedy", "surface greedy DP selected the final path")
          );
        }
        return {
          exact_entries: exactEntries.slice(),
          ...
          all_entries: allEntries.slice(),
          preferred_entries: allEntries.slice(),
          dict_head: promoted.length ? String(promoted[0].headword || surface) : surface,
          resolved_via: promoted.length ? "lemma_promoted" : "surface",
          lemma_hint_data: hintData,
          lemma_oracle_used: lemmaOracleUsed,
          lemma_oracle_outcome: promoted.length
            ? "lemma_promoted"
            : (lemmaOracleUsed ? "lemma_checked_no_promotion" : "surface_greedy"),
          exact_lemma_match_count: 0,
          resolution_meta: buildResolutionMeta(
            resolutionCategory,
            resolutionRouteCode,
            resolutionRouteSteps,
            promoted.length ? "lemma_promoted" : "surface",
            fillMode,
            lemmaOracleUsed,
            promoted.length
              ? "lemma_promoted"
    ...
    return {
      ...
      lemma_hint_data: hintData,
      lemma_oracle_used: lemmaOracleUsed,
      lemma_oracle_outcome: lemmaOracleUsed ? "no_lemma_match" : "no_match",
      exact_lemma_match_count: 0,
      resolution_meta: buildResolutionMeta(
        "",
        lemmaOracleUsed ? "surface_no_match_after_lemma_check" : "surface_no_match",
        lemmaOracleUsed
          ? [
              buildResolutionRouteStep("surface_exact_lookup", "surface exact lookup missed"),
              buildResolutionRouteStep("lemma_greedy", "lemma-aware greedy DP found no known dictionary path")
            ]
          : [
              buildResolutionRouteStep("surface_exact_lookup", "surface exact lookup missed"),
              buildResolutionRouteStep("surface_greedy", "surface greedy DP found no known dictionary path")
            ],
        "surface",
        String((fill && fill.mode) || "").trim().toLowerCase(),
        lemmaOracleUsed,
        lemmaOracleUsed ? "no_lemma_match" : "no_match"
      )
    };
  }
